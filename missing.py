...
def multilayer_perceptron(x, means, covs, p, gamma, n_distribution, weights, biases):
    ...
    def calculate_component(i, collect1, collect2):
        # where_isnan -> [wielkosc batcha, 784] - wartości True/False w zależności czy tam jest NaN

        # [wielkosc batcha, 784]

        # Robimy samplowanie, dla każdego przykładu 10 sampli
        # [x_1, x_2, NaN, x_4] -> [x_1, x_2, m_3, x_4]
        data_miss = tf.where(where_isnan, tf.reshape(tf.tile(means[i, :], [size[0]]), [-1, size[1]]), x_miss)
        # [x_1, x_2, NaN, x_4] -> [0., 0., cov_3, 0.]
        miss_cov = tf.where(where_isnan, tf.reshape(tf.tile(covs_[i, :], [size[0]]), [-1, size[1]]),
                            tf.zeros([size[0], size[1]]))
        # Po samplowaniu [wielkosc batcha, 784, 10]

        # cov * NR(mean / cov)
        layer_1_m = tf.relu(tf.add(tf.matmul(data_miss, weights['h1']), biases['b1']))
        # Zrobiliśmy aktywacje pierwszej warstwy [wielkosc batcha, 784, 10]
        layer_5 = np.mean(layer_5, 2)
